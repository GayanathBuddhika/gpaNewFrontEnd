...
export class MainResultComponent implements OnInit {
  ...
  gpa3Year: number = 0;
  gpa4Year: number = 0;
  totalCreditYear1: number = 0;
  totalCreditYear2: number = 0;
  totalCreditYear3: number = 0;
  totalCreditYear4: number = 0;
  ...
  getResultByEpNumber() {
    ...
    this.resultViewService.getResultByEpNumber("EP2032").subscribe(data => {
      ...
      console.log(this.results);
      console.log(this.results[0].studentCourse.student.degreeProYear);
  ...
  calculateGpa(){
      ...
      if (this.results[0].studentCourse.student.degreeProYear === 3) {
      ...
      if (this.results[0].studentCourse.student.degreeProYear === 4) {
  ...
  catogariceSubjectsEyerBy(results: Result[]) {
    ...
    results.forEach((result, i) => {
      let codeNumber = result.studentCourse.degreeCourse.course.courseCode.match(/\d/g);
      switch (codeNumber[0]) {
        case "1":
  ...
  findGpaInYear(results: Result[]) {
    ...
    results.forEach(result => {
      ...
      console.log("first year creadit point",result.studentCourse.degreeCourse.course.name, creditPoint, codeNumber);
      // get total creadit point
      sumOfCredits += creditPoint;
      console.log("first year creadit point", sumOfCredits);
    ...
    console.log("total value of (credit point * grade point) ", sumCpGp);
    // calculate gpa
  ...
  calculateFinalGpaForFourYeardf() {
    ...
    console.log("final gpa for four year  ************", sumOfatp, sumOfat )
    this.finalGpa = sumOfatp / sumOfat;
    this.calFianalGpa = true;
